...
`set_node_audio_properties()` remains the typed low-level `SPA_PARAM_Props` layer. Effective endpoint controls use the focused `set_node_volume()` and `set_node_mute()` helpers instead: they load WirePlumber's `mixer-api`, write through its cubic-scale `set-volume` action, and confirm the effective value exposed by its `get-volume` action. A supported mixer is projected as a synthetic node `ParameterValue` named `Mixer`, containing the existing `AudioPropertiesValue` type and explicit `rw` permissions. This keeps device-route controls distinct from raw node Props without changing runtime-value schema v1. Volume values must be finite and non-negative, mute must be a boolean, and volume confirmation uses an explicit bounded tolerance for hardware quantization.
